...
## FEMA NRI Data Dictionary

- **GEOID**: Unique 10-digit Census identifier for the town (FIPS code), a concatenation of the state (50), county (3 digits), and place (5 digits) codes.
- **town_name**: Full Census town name (NAMELSAD) used for joins and display.

### Area-Weighted Flood Loss Variables

**Note**: All values are weighted by the proportion of tract area intersecting each town.

- **IFLD_EALT_weighted**: Area-weighted total expected annual flood loss (USD/year).
- **IFLD_EALP_weighted**: Area-weighted expected annual population loss from flooding, including fatalities and injuries.
- **IFLD_EALPE_weighted**: Area-weighted expected annual population loss expressed in equivalent dollar terms (USD/year).
- **IFLD_EALB_weighted**: Area-weighted expected annual building loss from flooding (USD/year).
- **IFLD_EALA_weighted**: Area-weighted expected annual agricultural loss from flooding (USD/year).

### Area-Weighted Exposure Variables

- **POPULATION_weighted**: Area-weighted tract population estimate.
- **BUILDVALUE_weighted**: Area-weighted total building value exposure (USD).
- **AGRIVALUE_weighted**: Area-weighted total agricultural value exposure (USD).
- **AREA_weighted**: Area-weighted tract area from FEMA NRI data (square miles).
- **ALAND_sqmi**: Census-derived town land area converted from square meters to square miles (included to validate the area-weighting process against FEMA NRI tract geometry aggregation).

### Normalized Expected Annual Loss Metrics

- **EAL_per_capita**: Expected annual flood loss divided by area-weighted population (USD/person/year).
- **EAL_per_building_value**: Expected annual flood loss divided by total building value exposure.
- **EAL_per_agricultural_value**: Expected annual flood loss divided by total agricultural value exposure.

### Spatial Weighting Variable

- **weight**: Sum of normalized tract-overlap weights contributing to the town. Values near 1 indicate complete tract coverage. Values may exceed 1 because multiple tracts contribute to a single town. Used internally to calculate weighted averages for normalized FEMA scores.

### Area-Weighted FEMA NRI Index Variables

**Note**: These variables are area-weighted averages of FEMA National Risk Index standardized scores aggregated from census tracts to towns. All scores are normalized FEMA indices generally scaled from 0–100, where higher values indicate greater magnitude of the measured characteristic.

- **RISK_SCORE_avg**: Area-weighted average FEMA National Risk Index risk score. This is FEMA’s composite risk metric incorporating expected annual loss, social vulnerability, and community resilience. Higher values indicate greater overall flood-related risk.

- **SOVI_SCORE_avg**: Area-weighted average FEMA Social Vulnerability score. Measures demographic and socioeconomic vulnerability factors associated with disaster impacts and recovery challenges. Higher values indicate greater social vulnerability.

- **IFLD_RISKS_avg**: Area-weighted average FEMA flood-specific risk score. Represents FEMA’s normalized estimate of flood hazard risk severity. Higher values indicate greater modeled flood risk.

- **RESL_SCORE_avg**: Area-weighted average FEMA community resilience score. Represents the estimated ability of a community to prepare for, respond to, and recover from hazard events. Higher values indicate greater resilience and adaptive capacity.

- **CRF_VALUE_avg**: Area-weighted average FEMA Community Risk Factor score. Represents FEMA’s estimate of community-level conditions associated with hazard impacts and vulnerability. Higher values indicate greater community-level risk conditions.
