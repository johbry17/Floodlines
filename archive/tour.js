...
//
//   Interactive guided walkthrough for the Floodlines dashboard using Shepherd.js.
//
//   Responsibilities
//   • Prompt first-time visitors to take the guided tour
//   • Load Shepherd.js assets (with CDN fallback)
//   • Coordinate dashboard interactions throughout the tour
//   • Highlight interface elements and manage guided navigation
//   • Restore the dashboard to its default state when the tour ends
//
//   Design notes
//   • Implemented as a self-contained module to avoid polluting the global scope.
//   • Uses Promise-based helper utilities to sequence dashboard interactions.
//   • Exposes a minimal public API (restartTour / FloodlinesTour) for external use.
//   • Originally generated with AI (Claude), then extensively revised, expanded,
//     and integrated into the project architecture.
// =============================================================================
...
(function () {
  ...
  // Internal runtime state used to prevent overlapping tours,
  // coordinate cleanup, and support safe restarts.
  const state = {
    starting: false,
    active: false,
  ...
  /* ── time utilities ──────────────────────────────────────────────────────── */
  // The tour prompt is shown only once per user,
  // and only if the user hasn't seen it in the last 30 days.
  ...
  function markPrompted() {
    localStorage.setItem(PROMPT_KEY, String(msNow()));
  }
  function markSeen() {
    localStorage.setItem(SEEN_KEY, String(msNow()));
  }

  /* ── asset loaders ───────────────────────────────────────────────────────── */
  // Shepherd is loaded on demand so users who skip the tour don't download
  // unnecessary assets. Local copies are preferred, with CDN fallback.
  ...
  // These functions manipulate the dashboard to demonstrate its features during the tour.
  // They return Promises that resolve after a delay to allow animations and transitions to complete.
  ...
  function switchPrimaryModel(overlayValue) {
    ...
    var input = group.querySelector(
      'input[data-overlay="' + overlayValue + '"]',
    );
  ...
  function switchChoropleth(overlayValue) {
    var btn = document.querySelector(
      '#choropleth-control button[data-overlay="' +
        overlayValue +
        '"],' +
        '#choropleth-control-special button[data-overlay="' +
        overlayValue +
        '"]',
    );
  ...
  function selectTown(townName) {
    ...
    var opt = opts.find(function (o) {
      return o.text.trim().toLowerCase() === lower;
    });
    // 2. Exact value match
    if (!opt)
      opt = opts.find(function (o) {
        return o.value.toLowerCase() === lower;
      });
    ...
    var strip = function (s) {
      return s.replace(/\s+(city|town|village|gore|grant)$/i, "").trim();
    };
    if (!opt)
      opt = opts.find(function (o) {
    ...
    if (!opt)
      opt = opts.find(function (o) {
        return o.value !== "" && o.text.trim().length > 0;
      });
  ...
  function resetDashboard() {
    return switchChoropleth("Quadrants")
      .then(function () {
        return selectTown("top");
      }) // return to State in dropdown, which resets the stats card and map zoom
      .then(function () {
      ...
      .then(function () {
        return deactivateContextLayer("Population");
      })
      .then(function () {
        return deactivateContextLayer("Funding Bubble");
      })
      .then(function () {
        return deactivateContextLayer("River Corridors");
      })
      .then(function () {
        restoreSelectEl();
      });
  ...
  /* ── cleanup ─────────────────────────────────────────────────────────────── */
  // Reset the dashboard and remove any tour-related DOM elements or event listeners.

  function cleanup() {
    try {
      if (tour && typeof tour.hide === "function") tour.hide();
    } catch (_) {}
    tour = null;
    clearHighlight();
  ...
  /* ── Shepherd loader ─────────────────────────────────────────────────────── */
  // Load Shepherd.js assets, preferring local copies with CDN fallback.
  // Returns a Promise that resolves when Shepherd is available.
  ...
  function showLoadError(err) {
    ...
    document
      .getElementById("fl-tour-retry")
      .addEventListener("click", function () {
        d.remove();
        ensureShepherd().then(function () {
          if (tour) tour.start();
        });
      });
  ...
  /* ── welcome modal ───────────────────────────────────────────────────────── */
  // Show a modal dialog prompting the user to take the tour.
  // Only shown once per user, and only if the user hasn't seen it in the last 30 days.
  ...
  /* ── build tour ──────────────────────────────────────────────────────────── */
  // Build the Shepherd tour steps and return a Promise that resolves when the tour is ready.
  ...
  function buildTour() {
    ...
    return Promise.all([
      ...
      s({
        ...
        beforeShowPromise: function () {
          return switchChoropleth("Quadrants")
            ...
            .then(function () {
              setHighlight(mapEl);
            }); // introduce map and quadrants view
        },
        when: {
          hide: function () {
            clearHighlight();
      ...
      s({
        id: "quadrant-button",
        text: "<p>This button resets the map to the Quadrant view at any time, useful after exploring other overlays.</p>",
        attachTo: quadrantsBtn
          ? { element: quadrantsBtn, on: "bottom" }
          : undefined,
        beforeShowPromise: function () {
          scrollTo(quadrantsBtn);
          return delay(300).then(function () {
            setHighlight(quadrantsBtn);
          }); // introduce quadrants button
        },
        when: {
          hide: function () {
            clearHighlight();
      ...
      s({
        ...
        text:
          ...
          "<p>You can weight total expected losses, losses per person, or FEMA&rsquo;s own composite index. Each choice produces a different map, and a different answer to who is most at risk.</p>",
        attachTo: modelGroup ? { element: modelGroup, on: "top" } : undefined,
        beforeShowPromise: function () {
          scrollTo(modelGroup);
          return delay(400)
            .then(function () {
              setHighlight(modelGroup);
              return delay(400);
            }) // show controls and switch a model to demonstrate use
            .then(function () {
              return switchPrimaryModel("FEMA Risk Index");
            });
        },
        when: {
          hide: function () {
      ...
      s({
        id: "model-change",
        text:
          "<p>Watch the map as the model switches.</p>" +
          "<p>Different definitions of risk produce different rankings. Some towns rise, others fall. The model you choose shapes which towns appear most vulnerable.</p>",
        attachTo: mapEl ? { element: mapEl, on: "bottom" } : undefined,
        beforeShowPromise: function () {
          scrollTo(mapEl);
          return delay(400)
            .then(function () {
              setHighlight(mapEl);
            }) // switch models while map is highlighted so the connection is clear
            .then(function () {
              return delay(300);
            })
            .then(function () {
            ...
            .then(function () {
              return switchPrimaryModel("Total Risk");
            });
        },
        when: {
          hide: function () {
      ...
      s({
        ...
        beforeShowPromise: function () {
          return delay(400)
            ...
            }) // then scroll to choropleth controls and activate the Gap view
            .then(function () {
              setHighlight(choroplethCtrl);
            })
            .then(function () {
              return switchChoropleth("Funding Gap");
            });
        },
        when: {
          hide: function () {
      ...
      s({
        id: "funding-gap-map",
        text:
          "<p>The <strong>Funding Gap</strong> is the distance between a town&rsquo;s modeled need and the mitigation funding it has received.</p>" +
          "<p>Towns in red rank much higher on need than on funding. Many have no recorded federal mitigation investment at all.</p>" +
          "<p><b>This is the project&rsquo;s central question:</b> where does need exceed investment? Can we address these gaps before disaster strikes?</p>",
        attachTo: mapEl ? { element: mapEl, on: "bottom" } : undefined,
        beforeShowPromise: function () {
          scrollTo(mapEl);
      ...
      s({
        id: "scatterplot",
        text:
          "<p>Each bubble is a Vermont town. Need is shown on the horizontal axis, funding on the vertical. <strong>Bubble size reflects population.</strong></p>" +
          "<p>The model controls below are synchronized with the map above. Watch how towns move as the definition of risk changes.</p>" +
          "<p>If funding tracked need closely, the bubbles would cluster around a diagonal line. They don&rsquo;t.</p>",
        attachTo: plotContainer
          ? { element: plotContainer, on: "top" }
          : undefined,
        beforeShowPromise: function () {
          // scrollTo(plotContainer);
          return delay(500)
            .then(function () {
              setHighlight(modelGroupSecondary);
            }) // highlight the secondary model selector group before switching models so the connection is clear
            .then(function () {
              scrollTo(modelGroupSecondary);
            })
            .then(function () {
              return delay(400);
            })
            ...
            .then(function () {
              scrollTo(plotContainer);
            }) // highlight the plot container and switch models to show impact
            .then(function () {
              setHighlight(plotContainer);
            })
            .then(function () {
              return switchSecondaryModel("Total Risk");
            })
            .then(function () {
              return delay(1000);
            })
            .then(function () {
              return switchSecondaryModel("Risk per Person");
            })
            .then(function () {
              return delay(1000);
            })
            .then(function () {
              return switchSecondaryModel("FEMA Risk Index");
            });
        },
        when: {
          hide: function () {
      ...
      s({
        id: "rankings",
        text: "<p>The rankings table lists every Vermont town sorted by the current model. Use it to identify specific communities or compare them side by side.</p>",
        attachTo: rankingsContainer
          ? { element: rankingsContainer, on: "top" }
          : undefined,
        beforeShowPromise: function () {
      ...
      s({
        id: "rankings-nav",
        text: "<p>These shortcuts jump to the top-ranked towns, the statewide average, or a selected town once you&rsquo;ve chosen one from the dropdown.</p>",
        ...
        beforeShowPromise: function () {
          ...
          return delay(400)
            ...
            .then(function () {
              var toggleEl = document.querySelector(
                "#toggle-relative-secondary",
              );
              var container = toggleEl
                ? toggleEl.closest('[class*="col-"]') || toggleEl.parentElement
                : null;
      ...
      s({
        ...
        text:
          ...
          "<p>River corridor boundaries, population bubbles, and funding markers help explain why a town lands where it does.</p>",
        attachTo: mapEl ? { element: mapEl, on: "bottom" } : undefined,
        beforeShowPromise: function () {
          ...
          return delay(400)
            .then(function () {
              return selectTown("top");
            }) // reset to State after town zoom
            .then(function () {
              setHighlight(contextControls);
            }) // show controls before activating so the connection is clear
            .then(function () {
              return delay(700);
            })
            .then(function () {
              return activateContextLayer("Population");
            })
            .then(function () {
              return delay(700);
            })
            .then(function () {
              scrollTo(mapEl);
              setHighlight(mapEl);
            }) // show map and activate more layers to demonstrate how context adds up
            .then(function () {
              return delay(400);
            })
            .then(function () {
              return activateContextLayer("River Corridors");
            })
            .then(function () {
              return delay(700);
            })
            .then(function () {
              return activateContextLayer("Funding Bubble");
            })
            .then(function () {
              return delay(700);
            });
  ...
  // Start the tour if it isn't already running.
  // Returns immediately if the tour is already active or starting.
  ...
  /* ── restartTour (safe) ──────────────────────────────────────────────────── */
  // Restart the tour if it is already running, or start it if not.
  function restartTour() {
    if (state.starting) {
      state.requestedRestart = true;
      return;
    }
    if (state.active && tour) {
      state.requestedRestart = true;
      tour.cancel();
      return;
    }
  ...
  /* ── auto-prompt on load ─────────────────────────────────────────────────── */
  // Show the welcome modal on page load if the user hasn't seen it in the last 30 days.
  ...
  /* ── runtime styles (tour.css is the canonical source; these are reliable fallbacks) ── */
  // Shepherd.js loads asynchronously, so the tour highlight ring and modal backdrop
  // need to be styled immediately to avoid a flash of unstyled content. The modal card
  // is also styled here as a critical fallback so it is always visible even if tour.css is late.
  ...
  injectStyle(
    ...
    [
      ...
      /* modal card — critical fallback so card is always visible even if tour.css is late */
      ".fl-tour-modal-card{",
      ...
      "box-shadow:0 0 0 1px rgba(0,0,0,0.08),0 8px 24px rgba(0,0,0,0.30),0 24px 64px rgba(0,0,0,0.22);",
      "font-family:inherit;}",
      ".fl-tour-modal-header h2{margin:0 0 10px;font-size:1.2rem;font-weight:700;color:#0d1a2e;}",
      ".fl-tour-modal-body p{margin:0 0 10px;color:#3a3a3a;line-height:1.6;font-size:0.95rem;}",
      ".fl-tour-modal-footer{margin-top:22px;display:flex;justify-content:flex-end;gap:10px;}",
      /* buttons */
      ".fl-tour-btn{display:inline-block;padding:8px 18px;border-radius:5px;font-size:0.92rem;",
      "font-weight:600;cursor:pointer;border:none;line-height:1.4;font-family:inherit;}",
      ".fl-tour-btn-primary{background:#0085a1;color:#fff;}",
      ".fl-tour-btn-ghost{background:transparent;color:#555;border:1px solid #d0d0d0 !important;}",
    ].join(""),
  );

  /* ── public API ──────────────────────────────────────────────────────────── */
  // Expose the tour API to the global scope so it can be triggered from other scripts.
